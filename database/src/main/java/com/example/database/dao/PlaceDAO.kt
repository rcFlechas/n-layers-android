...
interface PlaceDAO {
    ...
    fun getPlacesAll(): Single<List<PlaceWithVehicleEntity>>

    @Transaction
    @Query("SELECT * FROM placeEntity WHERE plac_state =:state")
    fun getPlacesAllByState(state: State): Single<List<PlaceWithVehicleEntity>>

    @Query("SELECT * FROM placeEntity WHERE plac_id = :id")
    fun getPlaceById(id: Long) : Maybe<PlaceWithVehicleEntity>
    ...
    fun updatePlace(id: Long, freeDate: Date, state: State): Completable
}